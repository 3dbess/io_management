...
def cscan_algorithm(initial_position, requests):
    ...
    # If not already at the end, jump to the end
    if 199 not in sorted_requests:
        order_served.append(199)
        total_head_movement += abs(199 - initial_position)
        initial_position = 199

    # If not already at the beginning, jump to the beginning
    if 0 not in sorted_requests and initial_position != 0:
...
def plot_graph(order_served):
    ...
    plt.title('CSCAN (Circular SCAN) Disk Scheduling Algorithm')
...
if __name__ == "__main__":
    ...
    # Run CSCAN algorithm
    total_head_movement, order_served = cscan_algorithm(initial_position, requests)
